...
to a simulation mode whenever `pigpio` isn't connected, so the full web UI and
job pipeline run on any machine with Python 3.
...
On the real Pi: `sudo pigpiod` first, then `python run.py` — see the
[README](README.md).
...
- **Simulation must keep working.** Don't let a change make `pigpio` a hard
  import requirement; the graceful fallback in `hardware/executor.py` is what
  keeps the project contributable without hardware.
- **Keep the UI phone-first.** The framer drives this from a phone at the
  sawhorses; test layouts narrow.
- Hardware/GPIO changes: state what you tested on (Pi model, driver board) —
